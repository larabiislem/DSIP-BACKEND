...
class RawTransaction(Base):
    ...
    universal_transaction_number: Mapped[str | None] = mapped_column(String(64), index=True, nullable=True)
    reversal_flag: Mapped[int | None] = mapped_column(Integer, nullable=True)
    svfe_trace_number: Mapped[str | None] = mapped_column(String(64), nullable=True)
    svfe_message_type: Mapped[str | None] = mapped_column(String(32), nullable=True)
    response_code: Mapped[str | None] = mapped_column(String(16), nullable=True)
    svfe_transaction_type: Mapped[str | None] = mapped_column(String(32), nullable=True)
    svfe_response_code: Mapped[str | None] = mapped_column(String(32), nullable=True)
    completion_status: Mapped[int | None] = mapped_column(Integer, nullable=True)
    stood_in_for: Mapped[int | None] = mapped_column(Integer, nullable=True)
    issuer_posted: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ts: Mapped[datetime] = mapped_column(DateTime(timezone=False), index=True, nullable=False)
    gap_to_next_sec: Mapped[float | None] = mapped_column(Float, nullable=True)
    gap_label: Mapped[str | None] = mapped_column(String(64), nullable=True)
    local_expected_gap_sec: Mapped[float | None] = mapped_column(Float, nullable=True)
    local_ceiling_sec: Mapped[float | None] = mapped_column(Float, nullable=True)
    duration_to_expected_ratio: Mapped[float | None] = mapped_column(Float, nullable=True)
    hour_of_day: Mapped[float | None] = mapped_column(Float, nullable=True)
    day_of_week: Mapped[float | None] = mapped_column(Float, nullable=True)
    is_active_hours: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    minutes_from_open: Mapped[float | None] = mapped_column(Float, nullable=True)
    minutes_from_close: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class HourlyPrediction(Base):
    ...
    true_downtime_sec: Mapped[float | None] = mapped_column(Float, nullable=True)
    true_availability_pct: Mapped[float | None] = mapped_column(Float, nullable=True)
    error_abs_sec: Mapped[float | None] = mapped_column(Float, nullable=True)
